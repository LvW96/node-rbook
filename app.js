...
/*app.get('/book/list',(req,res) => {
  ...
  conn.query('select * from book',(err,results) => {
    if (err){
      ...
      res.json({
        error_code: 0,
        data:results//数据库查询结果返回到result中
      })
    }
    conn.end()
  })
...
function randomArray(n,l){
  let ranArray = []
  for (let i = 0;i < n; i++){
    ranArray.push(Math.floor(Math.random() * l))
  }
  return ranArray
}
...
function createCategoryId(n) {
  ...
  for (let i = 0;i < n;i++){
    // 随机抽取分类,获取的随机数不能重复
    const ran = Math.floor(Math.random() * (arr.length - 1))
    // 获取分类的序号
    results.push(arr[ran])
    // 将已经获取的随机数取代
...
app.get('/book/home',(req,res) => {
  ...
  conn.query('select * from book where cover !=\'\'',(err,results) => {
    ...
    //console.log(randomArray(20,length))
    // 猜你喜欢模块中的数据
    randomArray(60,length).forEach(key => {
